...
class AuthViewModel: ObservableObject {
    ...
    private func openPhantomConnect() {
        ...
        if let decoded = Base58.decode(publicKeyBase58) {
            logger.info("Decoded key size: \(decoded.count) bytes, matches: \(decoded == kp.publicKey)")
        }
    ...
    private func handleConnectCallback(components: URLComponents) {
        ...
        do {
            ...
            Task {
                await authenticateWithSession(walletAddress: publicKey, phantomSession: sessionString)
            }
